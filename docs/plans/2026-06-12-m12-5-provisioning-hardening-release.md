...
| **Date** | 2026-06-12 |
| **Status** | Software complete in the working tree (T12.5.1–T12.5.6, T12.5.8; suites green) but **UNCOMMITTED** on branch `m12-2-teleop-estop-video`. Provisioning screen + native-notification wiring integrated into the App shell (2026-06-12). T12.5.6 native notifications now deliver for real (`@tauri-apps/plugin-notification` + Rust `tauri-plugin-notification` installed/registered/permission-gated). T12.5.7 E2E honestly downgraded to manual/host-marked. **T12.5.9 V1 release gate OPEN** — perf targets ⬜ pending on hardware. |
