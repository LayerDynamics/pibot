...
| **Date** | 2026-06-11 |
| **Status** | Software shipped (T11.1–T11.3, T11.5, gate-green); HIL pending hardware (T11.4 multi-task demos+fine-tune, T11.6 autonomy E2E sign-off — runbooks/sign-off written PENDING). **SPEC-2 software complete.** Follow-on (committed separately): closed-loop autonomy moved **in-process inside pibotd** (`agent/autonomy.py` `AutonomyController` + `POST/DELETE /autonomy`; `pibot autonomy --run` is now a thin client). This closes T11.1's telemetry gap — the policy-link block is fed live, so `pibot monitor` shows it during a run — and makes host-side drop-to-stop real (off-thread inference + the agent's deadman ticker). Single transport, single `AgentSafety`. |
