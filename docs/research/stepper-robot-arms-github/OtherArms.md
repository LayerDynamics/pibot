...
| **Kinematics seam** | `pibot/arm/kinematics.py` | `JointSolver` Protocol; **`DirectSolver`** (joint-angle pass-through, range-checked); **`NamedPoseSolver`** (static preset poses); **`ForwardKinematics`** (joints → EE pose, M-ARM-3); **`IKSolver`** (EE pose → joints, M-ARM-4 — numeric ikpy IK over the in-tree geometry, reachability + joint-limit checked, raises on a miss). Both numeric solvers lazy-import ikpy/numpy behind the optional `pibot[arm-ik]` extra; the module stays numpy-free at import (NFR-2). |
